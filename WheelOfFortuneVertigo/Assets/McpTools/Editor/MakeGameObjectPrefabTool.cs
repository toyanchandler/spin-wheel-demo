...
namespace Vertigo.Wheel.McpTools
{
    ...
    public static class MakeGameObjectPrefabTool
    {
        public static object HandleCommand(JObject @params)
        {
            ...
            if (string.IsNullOrWhiteSpace(prefabPath)) return new ErrorResponse("'prefabPath' is required and must be under Assets/.");

            if (!prefabPath.StartsWith("Assets/", StringComparison.Ordinal) || !prefabPath.EndsWith(".prefab", StringComparison.OrdinalIgnoreCase)) return new ErrorResponse("'prefabPath' must be an Assets-relative .prefab path.");
            ...
            try
            {
                ...
                if (!success || prefab == null) return new ErrorResponse($"Unity failed to save prefab '{prefabPath}'.");
